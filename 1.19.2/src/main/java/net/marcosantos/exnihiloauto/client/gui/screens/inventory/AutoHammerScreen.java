...
public class AutoHammerScreen extends AbstractContainerScreen<AutoHammerMenu> {
	...
	public static final ResourceLocation BACK = new ResourceLocation(ExNihiloAuto.MODID, "textures/gui/auto_hammer.png");
		...
		if (this.hoveredSlot != null) {
			if (!this.hoveredSlot.hasItem()) {
				if (this.hoveredSlot.index == 1) {
					this.renderTooltip(guiGraphics, Component.translatable("hammer.input"), mouseX, mouseY);
				} else if (this.hoveredSlot.index == 0) {
					this.renderTooltip(guiGraphics, Component.translatable("hammer.hammer"), mouseX, mouseY);
				}
			} else {
	...
	@Override
	public void renderBg(@Nonnull PoseStack guiGraphics, float partialTicks, int mouseX, int mouseY) {
	...
	private void renderHammerTimer(PoseStack guiGraphics) {
		...
		if (ExNihiloAuto.HAS_PATCHOULI) {
			if (mouseX > this.leftPos + 181 && mouseX < this.leftPos + 181 + 18 && mouseY > this.topPos + 69 && mouseY < this.topPos + 69 + 16) {
				...
				PatchouliAPI.get().openBookEntry(
						...
						new ResourceLocation(ExNihiloAuto.MODID, "autohammer"),
						0);
				return true;
			}
		}

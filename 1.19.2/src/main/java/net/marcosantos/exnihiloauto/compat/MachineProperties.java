...
public class MachineProperties implements ICustomComponent {
	...
	public void render(PoseStack guiGraphics, IComponentRenderContext iComponentRenderContext, float v, int i, int i1) {
		int index = 0;
		for (String s : this.text) {
			...
			for (FormattedCharSequence iReorderingProcessor : fontRenderer.split(Component.literal(parse), 145)) {
				fontRenderer.draw(guiGraphics, iReorderingProcessor, x, y + index * (fontRenderer.lineHeight + 3), 0);
				index++;
			}
		}
	}
	...
	public void onVariablesAvailable(UnaryOperator<IVariable> unaryOperator) {
		String machineType = unaryOperator.apply(IVariable.wrap("#machine_type#")).asString();
